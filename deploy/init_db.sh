...
# 作用：创建生产库 + 应用 V1 建表 / V2 分类 / V3 演示商品 / V4 店铺 / V5 全文检索
...
for sql in V1__init_schema.sql V2__seed_data.sql V3__demo_products.sql V4__merchant_shop.sql V5__zhparser_fulltext.sql V6__ai_knowledge.sql; do
  echo "==> 应用 ${sql}"
...
#   1. 把 db/ 目录下 V1/V2/V3/V4 四个 SQL 传到服务器
#   2. 执行：
